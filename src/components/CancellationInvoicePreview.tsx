...
interface CancellationInvoicePreviewProps {
...
export const CancellationInvoicePreview: React.FC<CancellationInvoicePreviewProps> = ({ data }) => {
  ...
  return (
    <div className="invoice-preview bg-white p-4 max-w-4xl mx-auto" id="cancellation-invoice-preview" style={{ fontSize: '10px' }}>
      {/* Header */}
      <div className="border-2 border-black mb-0">
        ...
        <div className="text-center py-2 border-b-2 border-black">
          <h2 className="font-bold" style={{ fontSize: '12px' }}>INVOICE</h2>
        </div>
          ...
          <table className="w-full" style={{ tableLayout: 'fixed', fontSize: '10px' }}>
            ...
            <tbody>
              <tr className="border-b border-black">
                ...
                <td className="border-r border-black p-1" style={{fontWeight: 'bold'}}>Original Amount</td>
                <td className="p-1 text-center">{formatAmount(data.originalBookingAmount)}</td>
              </tr>
              <tr>
            ...
            <tbody>
              <tr className="border-b border-black">
                ...
                <td className="border-r border-black p-1">
                  <div className='font-semibold'>Original Booking Amount</div>
                  <div className="pl-2">Total Booking Value</div>
                </td>
                <td className="p-1 text-right">
                  <div className="mb-2">&nbsp;</div>
                  <div style={{paddingRight: '10px'}}>{formatAmount(data.originalBookingAmount)}</div>
              ...
              <tr className="border-b border-black">
                ...
                <td className="p-1 text-right">
                  <div className="mb-2">&nbsp;</div>
                  <div style={{paddingRight: '10px'}}>{formatAmount(data.totalPaid)}</div>
                  
                </td>
              </tr>
              <tr className="border-b border-black">
                <td className="border-r border-black p-1 text-center">3</td>
                <td className="border-r border-black p-1">
                  <div className='font-semibold'>Cancellation Charges</div>
                  <div className="pl-2">{data.cancellationReason}</div>
                </td>
                <td className="p-1 text-right">
                  <div className="mb-2">&nbsp;</div>
                  <div style={{paddingRight: '10px'}}>{formatAmount(data.cancellationCharges)}</div>
                  
                </td>
              </tr>
              <tr>
                <td className="border-r border-black p-1 text-center">4</td>
                <td className="border-r border-black p-1">
                  <div className='font-semibold'>Refund Status</div>
                  <div className="pl-2">As per cancellation policy</div>
                </td>
                <td className="p-1 text-right">
                  <div className="mb-2">&nbsp;</div>
                  <div style={{paddingRight: '10px', fontWeight: 'bold'}}>No refund on cancellation</div>
        ...
        <div className="border-b-2 border-black" style={{ height: '320px' }}>
          {/* Expanded empty space (removed signature section) */}
        </div>
              ...
              <tr>
                <td className="border-r border-black" style={{ width: '50%', verticalAlign: 'top', padding: '3px 4px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', marginTop: '0px' }}>
                    <span className="font-semibold">Balance Amount in Words : Rs</span>
                    <span style={{ marginLeft: '4px', borderBottom: '1px solid black', flex: '1', textAlign: 'center', paddingBottom: '2px' }}>
                      {numberToWords(Math.floor(data.totalPaid - data.cancellationCharges))}
                    </span>
                  </div>
                </td>
                <td style={{ width: '50%', padding: '3px 4px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <span style={{paddingRight: '10px'}}>{formatAmount(data.totalPaid - data.cancellationCharges)}</span>
                  </div>
                </td>
              </tr>
            </tbody>
        ...
        <div className="text-center p-2 border-b-2 border-black">
          <p style={{ fontSize: '10px' }}>Thank you for your understanding.</p>
        </div>
