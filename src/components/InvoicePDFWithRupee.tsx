...
// Import fonts as base64 to avoid CORS issues
const NOTO_SANS_REGULAR_BASE64 = '/fonts/NotoSans-Regular.ttf';
const NOTO_SANS_BOLD_BASE64 = '/fonts/NotoSans-Bold.ttf';

// Initialize fonts with proper error handling
let fontsRegistered = false;

const registerFontsWithRetry = async () => {
  if (fontsRegistered) return true;
  
  const attempts = [
    // Attempt 1: Local files with absolute URL
    {
      name: 'Local (absolute)',
      regular: `${window.location.origin}/fonts/NotoSans-Regular.ttf`,
      bold: `${window.location.origin}/fonts/NotoSans-Bold.ttf`
    },
    // Attempt 2: CDN with working CORS
    {
      name: 'CDN (unpkg)',
      regular: 'https://unpkg.com/noto-sans@1.0.0/fonts/NotoSans-Regular.ttf',
      bold: 'https://unpkg.com/noto-sans@1.0.0/fonts/NotoSans-Bold.ttf'
    },
    // Attempt 3: Alternative CDN
    {
      name: 'CDN (cdnjs)',
  ...
  for (const attempt of attempts) {
    try {
      ...
      console.log(`✅ Fonts registered successfully using: ${attempt.name}`);
      fontsRegistered = true;
      return true;
    } catch (error) {
      console.warn(`Failed to register fonts from ${attempt.name}:`, error);
    }
  }
  
  console.warn('⚠️ All font registration attempts failed, using system fonts');
  return false;
};

// Try to register fonts immediately
registerFontsWithRetry();

// Create styles - use Helvetica as fallback
...
// Custom component for Rupee symbol with fallback
const RupeeAmount: React.FC<{ amount: string | number; prefix?: boolean }> = ({ amount, prefix = true }) => {
  // Try to use actual rupee symbol, fallback to Rs. if it doesn't render
  const rupeeSymbol = '₹'; // This will work if font supports it
  const fallbackSymbol = 'Rs.'; // Fallback
  
  return (
    <Text>
      {prefix ? rupeeSymbol : ''}{amount}
    </Text>
  );
};
...
const InvoicePDFDocument: React.FC<InvoicePDFProps> = ({
  ...
  // Determine currency symbol to use
  const currencySymbol = fontsRegistered ? '₹' : 'Rs.';
          ...
          <View style={styles.content}>
            ...
            <View style={styles.table}>
              <View style={[styles.tableRow, styles.tableHeader]}>
                ...
                <Text style={[styles.tableColHeader, styles.colRate]}>Rate ({currencySymbol})</Text>
                <Text style={[styles.tableColHeaderLast, styles.colAmount]}>Amount ({currencySymbol})</Text>
              </View>
                ...
                <>
                  {charges.map((charge, index) => {
                    ...
                    return (
                      <View key={`charge-${index}`} style={styles.tableRow}>
                        ...
                        <Text style={[styles.tableCol, styles.colRate]}>{charge.unitAmount ? `${currencySymbol}${inr(charge.unitAmount)}` : '-'}</Text>
                        <Text style={[styles.tableColLast, styles.colAmount]}>
                          {isDiscount ? `-${currencySymbol}${inr(Math.abs(amount))}` : `${currencySymbol}${inr(amount)}`}
                        </Text>
                      </View>
                    );
                  })}
                  ...
                  {financials && charges.length > 0 && (
                    <View style={[styles.tableRow, styles.subtotalRow]}>
                      ...
                      <Text style={styles.subtotalValue}>{currencySymbol}{inr(financials.grossTotal)}</Text>
                    </View>
                  )}
                  ...
                  {payments.map((payment, index) => (
                    <View key={`payment-${index}`} style={styles.tableRow}>
                      ...
                      <Text style={[styles.tableColLast, styles.colAmount]}>
                        {payment.paymentType === 'refund' ? `${currencySymbol}${inr(payment.amount)}` : `-${currencySymbol}${inr(payment.amount)}`}
                      </Text>
                    </View>
                  ))}
                  ...
                  {financials && payments.length > 0 && (
                    <View style={[styles.tableRow, styles.subtotalRow, { borderBottom: 0 }]}>
                      ...
                      <Text style={styles.subtotalValue}>-{currencySymbol}{inr(financials.paymentsTotal)}</Text>
                    </View>
                  )}
                </>
              )}
            ...
            {financials && (
              <View style={styles.summarySection}>
                <View style={styles.summaryRow}>
                  ...
                  <Text style={styles.summaryValue}>{currencySymbol} {inr(financials.grossTotal)}</Text>
                </View>
                <View style={styles.summaryRow}>
                  <Text style={styles.summaryLabel}>Total Payments:</Text>
                  <Text style={styles.summaryValue}>{currencySymbol} {inr(financials.paymentsTotal)}</Text>
                </View>
                <View style={styles.totalRow}>
                  <Text style={styles.totalLabel}>BALANCE DUE:</Text>
                  <Text style={styles.totalValue}>{currencySymbol} {inr(financials.balanceDue)}</Text>
                </View>
              </View>
            )}
...
export const InvoicePDFExport: React.FC<InvoicePDFProps & { children?: React.ReactNode; className?: string }> = (props) => {
  ...
  const handleClick = async () => {
    try {
      ...
      // If error, generate without custom fonts
      try {
        console.log('Generating PDF with system fonts...');
        await downloadInvoicePDF(pdfProps as InvoicePDFProps);
      } catch (fallbackError) {
